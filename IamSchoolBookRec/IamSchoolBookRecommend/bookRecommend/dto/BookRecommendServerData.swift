...
class BookRecommendServerData {
    private let iamSchoolAPI: IamSchoolAPI = IamSchoolAPI()
    private let baseURLString = "http://nany.synology.me/book-home.json"
    private let serverIdentifierParser = ServerIdentifierParser()
    private func bookRecommendURL() -> URL? {
        let components = URLComponents(string: baseURLString)
        return components?.url
    }
    
    var cellDatas: [BookRecommendationCellData]?
    ...
    func fetchDataFromServer(){
        ...
        iamSchoolAPI.fetchData(request: url) {
            [weak self]
            (bookRecommendResponseDtos: [BookRecommendResponseDto]?) in
            guard let bookRecommendResponseDtos = bookRecommendResponseDtos else {
                return
            }
            self?.cellDatas = self?.serverIdentifierParser.parse(datas: bookRecommendResponseDtos)
        }
    }
    
    subscript(indexPath: IndexPath) -> BookRecommendationCellData? {
        guard let cellDatas = cellDatas,
